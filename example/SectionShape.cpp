...
#include "SquareSectionView.h"
#include "graphicshexagonitem.h"

using namespace LCR;
using namespace MQGAPI;

SquareSectionView::SquareSectionView(double ins_diameter, double _septa, int _direction, QWidget *parent)
	: CollimatorHorizontalSectionView(parent)
	, hex_group(Q_NULLPTR)
	, base_rect(Q_NULLPTR)
...
void SquareSectionView::setCollimatorSize(const v3& coll_size)
{
	size = coll_size;
}

void SquareSectionView::setParameters(double _diameter, double _septa, int _direction)
{
	diameter = _diameter;
	septa = _septa;
	direction = _direction;
...
void SquareSectionView::updateBase()
{
	if (septa < 0) {
...
void SquareSectionView::buildHoles()
{
	if (hex_group) {
...
void SquareSectionView::drawCircle(double radius)
{
...
void SquareSectionView::draw_outside()
{
...
void SquareSectionView::build_horizontal()
{
	QPen pen(Qt::black);
	QPen pen2(Qt::red);
...
void SquareSectionView::build_vertical()
{
	QPen pen(Qt::black);
	QPen pen2(Qt::red);
	QBrush brush(Qt::white, Qt::SolidPattern);
